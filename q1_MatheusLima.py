...
user_accounts = user_accounts_dict()
# Criando transação
...
decision = lambda: (confirm_payment(), credit_activities_list.append(complete_transaction)) and \
                   fund_cash_activities_list.append(complete_transaction) if \
...
credit_activities = lambda: [
    create_transaction,
    request_credit_account_details,
    request_payment_from_bank,
    decision
]
credit_activities_list = credit_activities()
...
# Lista de atividades para o fluxo "Fund"
fund_cash_activities = lambda: [
    create_transaction,
    fund_trasnfer,
    bank_details,
    decision
...
fund_cash_activities_list = fund_cash_activities()
...
check_balance = lambda user, amount: user_accounts.get(user, {}).get("balance", 0) > amount if \
                type(amount) is int or type(amount) is float and amount > 0 else None
...
check_balance_and_check = lambda login, amount: (True, print("The user has sufficient balance...")) if \
                        check_balance(login, amount) else print("The amount cannot be less than 0!") if amount < 0 \
                        else (False, print("The user does NOT have sufficient balance..."))
...
debit = lambda user, amount, flow: user_accounts[user].update({"balance": user_accounts[user]["balance"] - amount}) if \
        ...
        complete_transaction in credit_activities_list or complete_transaction in fund_cash_activities_list \
...
choose_flow = lambda flow_type: execute_activities(cash_activities()) if flow_type == "cash" \
            else execute_activities(credit_activities_list) if flow_type == "credit" \
            else execute_activities(fund_cash_activities_list) if flow_type == "fund" \
...
num = lambda ls, amount: [float(amount)] if str(amount).replace('.', '', 1).replace('-', '', 1).isdigit() else []
check_empty_list = lambda ls: len(ls) == 0
...
check_balance_action = lambda auth, login, amount, flow: choose_flow(flow) and debit(login, amount, flow) \
                       if check_balance(login, amount) and auth and amount > 0 else None
